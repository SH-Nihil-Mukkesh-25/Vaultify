  ...
  if(rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()){
    ...
    if(valid){
      ...
      if(alarmActive && theftLockActive){
        ...
        doorOpen = !doorOpen;
        if(doorOpen){ doorServo.write(SERVO_UNLOCKED_POS); doorOpenTime=millis(); sendSms("Door unlocked."); sendToBackend("door_unlocked","RFID authorized"); }
        else { doorServo.write(SERVO_LOCKED_POS); sendSms("Door locked."); sendToBackend("door_locked","RFID authorized"); }
      }
    } else {
      shortBeep(3);
...
void handleDoorState(){
  if(doorOpen && (millis()-doorOpenTime > DOOR_AUTO_CLOSE)){
    ...
    sendSms("Door auto-locked."); sendToBackend("door_autolock","Auto-lock executed");
    updateLcdDisplay();
  }
}
