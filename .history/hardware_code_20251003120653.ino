...
const unsigned long LOG_COOLDOWN = 2000; // 2 seconds between logs
...
// -------------------- BACKEND LOGGING --------------------
void sendToBackend(String eventType, String detail, String metadata = ""){
  // Rate limiting
  if(millis() - lastLogSentTime < LOG_COOLDOWN) {
    Serial.println("Rate limit: skipping log");
    return;
  }
  ...
  String payload = "{\"event\":\""+eventType+"\",\"detail\":\""+detail+"\",\"timestamp\":\""+timestamp+"\"";
...
void handleMotionSensor(){
  ...
  if(acc > sq(ACCEL_THRESHOLD) && !alarmActive){
    ...
    String metadata = "{\"acceleration\":"+String(sqrt(acc), 2)+",\"threshold\":"+String(ACCEL_THRESHOLD)+"}";
    
    sendSms("ALERT: Theft detected!");
    sendToBackend("motion_alert","Possible intrusion detected", metadata);
  }
}
...
void handleRfidReader(){
  if(rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()){
    ...
    String metadata = "{\"card_uid\":\""+cardUID+"\",\"authorized\":"+String(valid ? "true" : "false")+"}";
    ...
    if(valid){
      ...
      if(alarmActive && theftLockActive){
        ...
        sendToBackend("alarm_deactivated","Theft alarm turned off by authorized card", metadata);
        ...
        if(doorOpen){ 
          ...
          sendToBackend("door_unlocked","Door unlocked by authorized card", metadata); 
        } else { 
          doorServo.write(SERVO_LOCKED_POS); 
          sendSms("Door locked."); 
          sendToBackend("door_locked","Door locked by authorized card", metadata); 
        }
      }
    } else {
      shortBeep(3);
      ...
      sendToBackend("rfid_invalid","Unauthorized card scan attempt", metadata);
      delay(2000);
    }
    updateLcdDisplay();
...
void handleDoorState(){
  if(doorOpen && (millis()-doorOpenTime > DOOR_AUTO_CLOSE)){
    doorServo.write(SERVO_LOCKED_POS); 
    doorOpen=false;
    String metadata = "{\"timeout_ms\":"+String(DOOR_AUTO_CLOSE)+"}";
    sendSms("Door auto-locked."); 
    sendToBackend("door_autolock","Auto-lock executed after timeout", metadata);
    updateLcdDisplay();
  }
}
...
void setup(){
  ...
  // Send system start log
  sendToBackend("system_start","Security system initialized");
